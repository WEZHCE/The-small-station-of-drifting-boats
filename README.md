...
**桌面与任务栏**
- 任务栏：开始菜单（系统与设备 / 网站设置：屏保时长·夜航·音效·开机动画 / 游客信息：第 N 次上船·来路·设备 / 关机）、cmd、帮助.chm、计算器、扫雷.EXE、日记.EXE、小站浏览器（只能访问船际网络认证站点：复古主页/BBS/气象台/日报，乱输网址有惊喜）、沙漏.EXE（像素沙漏倒计时，沙子流动可视化）、画板.EXE（24×24 像素画板：画笔/油漆桶/橡皮/撤销，画完一键下载 PNG）、太阳系.EXE（真实比例+真实轨道周期：左侧星球列表点谁去谁、拖动平移、滚轮缩放、点行星看资料并锁定镜头，缩到最小说明什么叫"空"）、倒数日.EXE（内置节日倒数含农历 + 自定义事件可写详细说明，存在本地，谁的日子近谁排前面）、图片搜索.EXE（按内容和标签找图：输入"猫/操场/云/毕业"这类关键词或点标签，命中高亮，点结果直达大图）、托盘点灯、真实时钟（夜航整栏换深夜配色）
- 仿真 cmd：启动 banner、`↑↓` 历史命令、`help / ver / echo / tree / start / sudo` 等一整套命令（彩蛋无数）
- 帮助.chm：全站彩蛋的官方使用手册（弹窗，可拖动；翻开前有一道"剧透警告"确认，可勾选不再问）
- 所有窗口都能拖标题栏移动、点击置顶

**内容板块**
- 关于我 / 船舱日记（GUI 日历 + 红点标记录日期，近况与日记统一时间线，项目带仓库链接）/ 相册（十五本板块相册，已开张九本：像素画集、捣鼓token、校园时光、船长的画、摄影作品、生活碎片、九小回忆、神秘一高、出去玩，其余六本虚位以待；主页面只陈列封面与简介，双击封面或点「翻开相册」在独立的 相册.EXE 窗口里查看全部图片；灯箱浏览，←→ 切换）/ 留言板（giscus + 全文搜索）
- 漂流瓶：一键从留言里随机捞一条陌生人的话
- 左下角三块面板：甲板音乐（像素频谱播放器，已上架十四首——初音ミク / 镜音铃 / 重音テト / GUMI / 洛天依等 VOCALOID，加上 Something Just Like This / See You Again /《玲音》OP Duvet 等英文歌与动漫主题曲，开站随机播放）+ 船长状态（课表 + 学生法定节假日双驱动的状态灯：地点 / 事情 / 心情，寒暑假、小长假、周末、调休补课自动切换）+ 灵动岛（实时显示游客正在浏览的板块 / 打开的应用 / 挂机状态，记录本次上船时长）
...
└── docs/            截图与相册图片（campus 校园实拍 / yigao 神秘一高 / trip 出去玩 / tokens 燃料账单 / art 船长的画 / photo 摄影作品 / life 生活碎片 / jx 九小回忆）
```
